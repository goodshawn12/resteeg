function CONFIG = resteeg_ANTNeuro(CONFIG)
...
[~,filename,~] = fileparts(CONFIG.filename);
if exist([CONFIG.filepath filesep filename '_import.set'],'file') && ~CONFIG.FORCE_RUN_IMPORT
    EEG = pop_loadset([CONFIG.filepath filesep filename '_import.set']);
    tmp = load([CONFIG.report.directory filesep 'config_import.mat']);
    CONFIG.rawinfo = tmp.config_import;
else
    [EEG, CONFIG] = prep_import(CONFIG);
    [EEG, CONFIG] = save_data(EEG,CONFIG,[filename '_import'],0);
end

% extract channels and save as .edf file
prep_ANTNeuro(EEG,[CONFIG.filepath filesep filename '_raw19.edf'])
...
if exist([CONFIG.filepath filesep filename '_prep.set'],'file') && ~CONFIG.FORCE_RUN_PREPROC
    EEG = pop_loadset([CONFIG.filepath filesep filename '_prep.set']);
    tmp = load([CONFIG.report.directory filesep 'config_prep.mat']);
    CONFIG.prep = tmp.config_prep;
else
    [EEG, CONFIG] = prep_proc(EEG,CONFIG);
    [EEG, CONFIG] = save_data(EEG,CONFIG,[filename '_prep'],1);
end

% extract channels and save as .edf file
prep_ANTNeuro(EEG,[CONFIG.filepath filesep CONFIG.filename '_post19.edf'])
