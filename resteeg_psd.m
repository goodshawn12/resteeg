...
config_name = {EEG.chanlocs.labels};
%config_mat = [2 4 7 9 11:15 17 19 21:25 27 29 31:35 36 37 39 40 42 44 45];
config_name = {EEG.chanlocs.labels};
%config_mat = [2 4 7 9 11:15 17 19 21:25 27 29 31:35 36 37 39 40 42 44 45];
config_mat = [2 4 7 9 11 15 12 13 14 17 19 21 25 22 23 24 27 29 31 35 32 33 34 36 40 37 39 42 44 45]
h = figure();
set(h, 'visible', 'off');
h.PaperUnits = 'inches';
h.PaperPosition = [0 0 8.5 11]*2;
for node = 1:30
    subplot(9,5,config_mat(node));
    plot(transpose(freqs(1:56)), spectra(node, 1:56));
    ax = gca;
    ax.XTick = 0:10:55;
    xlim([0, 55])
    ylim([-10, 18])
    grid ON
    title(config_name{node});
    if node < 27 
        %set(gca,'xtick',[],'ytick',[]);
        xlabel([]); ylabel([]);
...
saveas(h, 'frequences of each node', 'jpg')
h = figure()
set(h, 'visible', 'off')
h.PaperUnits = 'inches';
h.PaperPosition = [0 0 8.5 11]*2;
...
for node = 1:30
    subplot(9,5,config_mat(node));
    %spectrogram(EEG.data(node, :),500,250,500, 1e3,'yaxis')
    %spectrogram(EEG.data(node,:),window,noverlap,nfft, EEG.srate, 'yaxis');
    [s,f,t] = spectrogram(EEG.data(node,:),window,noverlap,nfft,EEG.srate);
        
    freq_range = 1:(find(f>50,1)-1);
        log_power = log(abs(s(freq_range,:)).^2);
        imagesc(t,f(freq_range),log_power); set(gca,'YDir','normal'); colorbar
...
saveas(h, 'frequency decomposition', 'jpg')
%plot(spectra, freqs)
